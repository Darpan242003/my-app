...
const Navbar = (props) => {
  ...
  // Function to determine navbar background color
  const getNavbarStyle = () => {
    if (props.navbarBg) {
      return { backgroundColor: props.navbarBg }
    }
    return {}
  }

  return (
    // <nav className={`navbar navbar-expand-lg navbar-${props.mode} bg-${props.navbarBg}`}>
    <nav
      className={`navbar navbar-expand-lg navbar-${props.mode} ${!props.navbarBg ? `bg-${props.mode}` : ''}`}
      style={getNavbarStyle()}
    >
    {/*change the navbar-light and bg-light properties with props to change light/dark using javascript template literals.*/}
